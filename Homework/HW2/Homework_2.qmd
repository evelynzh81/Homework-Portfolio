...
```{r}
setwd("C:/Users/evely/Documents/Math_3190")
flights_con = DBI::dbConnect(RSQLite::SQLite(),
                             "flights.sqlite")
# dbGetQuery(flights_con, "SELECT * FROM flights") |> head()
DBI::dbListTables(flights_con)
```
...
```{r}
#| cache: true
dbGetQuery(flights_con, "SELECT AVG(DISTANCE) FROM flights") #822.3565
dbGetQuery(flights_con, "SELECT MIN(DISTANCE) FROM flights") #21
dbGetQuery(flights_con, "SELECT MAX(DISTANCE) FROM flights") #4983
dbGetQuery(flights_con, "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT, MAX(DISTANCE)
                         FROM flights") # JFK, HNL
dbGetQuery(flights_con, "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT, MIN(DISTANCE)
                         FROM flights") # EWR JFK
dbGetQuery(flights_con, "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT, MIN(DISTANCE)
                         FROM flights
                         WHERE ORIGIN_AIRPORT OR DESTINATION_AIRPORT NOT LIKE '1%'")
dbGetQuery(flights_con, "SELECT airports.AIRPORT, flights.DISTANCE
                         FROM flights
                         INNER JOIN airports
                            ON airports.IATA_CODE = flights.ORIGIN_AIRPORT OR airports.IATA_CODE = flights.DESTINATION_AIRPORT
                         WHERE flights.DISTANCE = 21")

dbGetQuery(flights_con, "SELECT DISTINCT airports.AIRPORT, flights.DISTANCE
                         FROM flights
                         INNER JOIN airports
                            ON airports.IATA_CODE = flights.ORIGIN_AIRPORT OR airports.IATA_CODE = flights.DESTINATION_AIRPORT
                         WHERE flights.DISTANCE = 4983")
```
...
```{r}
delay_times = dbGetQuery(flights_con, "SELECT airlines.AIRLINE,
                         ARRIVAL_DELAY
                         FROM flights
                         INNER JOIN airlines
...
```{r}
ggplot(data = delay_times, aes(x=AIRLINE, y=ARRIVAL_DELAY)) +
  geom_boxplot() +
  scale_y_log10()+
...
--| echo: true
--| eval: true
--| output.var: airlines_avg_delay

SELECT airlines.AIRLINE as airline_fullname,
       AVG(flights.ARRIVAL_DELAY) AS avg_delay
FROM flights
INNER JOIN airlines
  ON airlines.IATA_CODE = flights.AIRLINE
...
```{r}
#| label: barplot
airlines_avg_delay = tibble(airlines_avg_delay)
head(airlines_avg_delay)

ggplot(data=airlines_avg_delay, aes(x=airline_fullname, y=avg_delay))+
...
**I think Hawaiian Airlines has the largest standard deviation in elapsed time because they fly within the contiguous United States and from Hawaii to the contiguous United States.**

Finally, when you are done with this assignment, it is good practice to close the connection to the flights database.

```{r}

flights_table = tbl(flights_con, 'flights')
airlines_table = tbl(flights_con, 'airlines')

flights_table = inner_join(airlines_table, flights_table, by = c('IATA_CODE'='AIRLINE'))

flights_summary = flights_table |>
  group_by(AIRLINE) |>
  summarize(sd_time = sd(ELAPSED_TIME),
            avg_time = mean(ELAPSED_TIME)) |>
  arrange(sd_time)
```

```{r}
#| label: barplot_elapsed_time

ggplot(data=flights_summary, aes(x=AIRLINE, y=sd_time))+
  geom_bar(stat='identity')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
```
